...
class EmployeesDataTable extends BaseDataTable
{
    ...
    public function query(User $model)
    {
        ...
        $users = DB::table('users')->where('users.company_id',\Auth()->user()->company_id)
            ->join('role_user', 'role_user.user_id', '=', 'users.id')
            ->leftJoin('employee_details', 'employee_details.user_id', '=', 'users.id')
            ->leftJoin('designations', 'employee_details.designation_id', '=', 'designations.id')
            ->join('roles', 'roles.id', '=', 'role_user.role_id')
            ->select('users.id', 'users.name', 'users.email', 'users.created_at', 'users.image', 'users.status','designations.name as designation_name')
        ...
        if ($request->cabang != 'all' && $request->cabang != '') {
            $users = $users->where('employee_details.cabang_id', $request->cabang);
        }
        if ($request->subcompany != 'all' && $request->subcompany != '') {
            $users = $users->where('employee_details.sub_company_id', $request->subcompany);
        }
        if ($request->wilayah != 'all' && $request->wilayah != '') {
            $users = $users->where('employee_details.wilayah_id', $request->wilayah);
        }
