...
class AdminDashboardController extends AdminBaseController
{
    ...
    public function index()
    {
        ...
        //         DB::raw('(select count(attendances.id) from `attendances` inner join users as atd_user on atd_user.id=attendances.user_id where DATE(attendances.clock_in_after_timezone) = CURDATE()  AND attendances.company_id = ' . $this->user->company_id . ' and atd_user.status = "active") as totalTodayAttendance'),
        ...
        $three_month_from_now = Carbon::now()->subMonths(3)->format('Y-m-d');
        $this->leaves = Leave::where('status', '<>', 'rejected')->whereDate('created_at','>=',$three_month_from_now)->get();
        // $this->leaves = [];
