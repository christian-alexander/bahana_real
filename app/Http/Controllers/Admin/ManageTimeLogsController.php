...
class ManageTimeLogsController extends AdminBaseController
{
    ...
    public function store(StoreTimeLog $request)
    {
        ...
        if (is_null($activeTimer)) {
            ...
            if ($request->hasFile('image')) {
                $timeLog->image = Files::upload($request->image, 'screenshot', 600);
            }
    ...
    public function edit($id)
    {
        ...
        if ($this->timeLog->task_id) {
            ...
            $this->list_member = $this->task->users->unique();
            // print_r($this->task->users[0]->id);die;
        } else {
            $this->project =  Project::findOrFail($this->timeLog->project_id);
            $this->timeLogProjects = Project::all();
            $this->timeLogProjects = $this->timeLogProjects->withCustomFields();
        }
        // dd($this->timeLog->user_id);
    ...
    public function update(UpdateTimeLog $request, $id)
    {
        ...
        $timeLog->checker_user_id = $this->user->id;
        $timeLog->save();
    ...
    {
        $project_time_log = ProjectTimeLog::find($id);
        $project_time_log->status = 'accepted';
        $project_time_log->save();

        return Reply::success('Successfully accepted time log');
    }
    public function reject_reason($id)
    {
        $this->timeLog = ProjectTimeLog::find($id);
        return view('admin.projects.time-logs.reject', $this->data);
    }
    public function reject(request $request, $id)
    {
        $project_time_log = ProjectTimeLog::find($id);
        $project_time_log->status = 'rejected';
        $project_time_log->reason = $request->reason;
        $project_time_log->save();

        return Reply::success('Successfully reject time log');
        // return $request->all();
        // $this->timeLog = ProjectTimeLog::find($id);
        // return view('admin.projects.time-logs.reject', $this->data);
    }
