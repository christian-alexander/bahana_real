...
class MemberLeavesController extends MemberBaseController
{
    ...
    public function store(StoreLeave $request)
    {
        // return $request->all();
        // khusus jika sakit
        if (isset($request->sakit) && !empty($request->sakit)) {
            $leave = new Leave();
            $leave->user_id = $request->user_id;
            $leave->leave_type_id = $request->leave_type_id;
            $leave->duration = 'single';
            $leave->leave_date = Carbon::now()->format('Y-m-d');
            $leave->is_sakit = '1';
            $leave->reason = $request->reason;
            $leave->status = $request->status;

            if ($request->hasFile('image')) {
                $leave->surat_keterangan_sakit = $request->image->hashName();
                $request->image->store('surat-keterangan-sakit');
            }
        ...
        if ($request->duration == 'multiple') {
            ...
            foreach ($dates as $date) {
                ...
                if (isset($request->budget) && !empty($request->budget)) {
                    $leave->budget_keuangan = $request->budget;
                }
                $leave->save();
            }
        } else {
    ...
    public function checkSakit()
    {
        try {
            // now - 1 day (yesterday)
            $yesterday = Carbon::now()->subDay()->format('Y-m-d');
            // get data
            $data = Leave::where('user_id', $this->user->id)
                ->where('is_sakit', 1)
                ->where('status', 'approved')
                ->where('leave_date', $yesterday)
                ->count();
            return $data;
        } catch (\Throwable $th) {
