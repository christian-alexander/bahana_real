...
class MemberAllTimeLogController extends MemberBaseController
{
    ...
    public function index()
    {
        if ($this->user->can('view_timelogs')) {
            ...
            $this->tasks = Task::join('task_users as tu', 'tu.task_id', '=', 'tasks.id')
                ->where('tu.user_id', '=', $this->user->id)
                ->select('tasks.*')
                ->get();
            ...
            $this->timeLogTasks = Task::join('task_users as tu', 'tu.task_id', '=', 'tasks.id')
                ->where('tu.user_id', '=', $this->user->id)
                ->select('tasks.*')
                ->get();
        }
    ...
    public function data(Request $request, $projectId = null, $employee = null)
    {
        ...
        $timeLogs->select('project_time_logs.id', $projectName, 'project_time_logs.start_time', 'project_time_logs.end_time', 'project_time_logs.total_hours', 'project_time_logs.total_minutes', 'project_time_logs.memo', 'project_time_logs.project_id', 'project_time_logs.task_id', 'users.name', 'project_time_logs.status', 'project_time_logs.reason');
