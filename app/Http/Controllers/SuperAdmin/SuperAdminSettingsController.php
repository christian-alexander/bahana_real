...
class SuperAdminSettingsController extends SuperAdminBaseController
{
    ...
    public function update(UpdateGlobalSettings $request, $id)
    {
        ...
        $setting->show_signup = $request->has('show_signup') && $request->input('show_signup') == 'on' ? 1 : 0;
        $setting->show_home = $request->has('show_home') && $request->input('show_home') == 'on' ? 1 : 0;
        $setting->show_menu = $request->has('show_menu') && $request->input('show_menu') == 'on' ? 1 : 0;
        $setting->single_sign_on = $request->has('single_sign_on') && $request->input('single_sign_on') == 'on' ? 1 : 0;
