...
class LeaveApproved extends Notification implements ShouldQueue
{
    ...
    private $cc;
    public function __construct(Leave $leave, $user, $approvedBy, $cc =[],$notif_to)
    {
        $this->leave = $leave;
        $this->cc = $cc;
        $this->user = $user;
    ...
    public function via($notifiable)
    {
        ...
        $msg = $this->user->name." mengajukan $getType->type_name diterima oleh ". $this->approvedBy->name;
        EmailToSend::saveData($this->notif_to,$msg);
        foreach ($this->cc as $email) {
            // get user
            $userAtasan = DB::table('users')->where('email', $email)->first();
            $msgAtasan = $this->user->name . " mengajukan $getType->type_name diterima oleh ". $this->approvedBy->name;
    ...
    public function toMail($notifiable)
    {
        ...
        return (new MailMessage)
            ...
            ->line($this->user->name . " mengajukan $getType->type_name diterima oleh ". $this->approvedBy->name);
    }
    ...
    public function toArray($notifiable)
    {
        ...
        $data['notif']['description'] = $this->user->name . " mengajukan $getType->type_name diterima oleh ". $this->approvedBy->name;
    ...
    public function toOneSignal($notifiable)
    {
        ...
        return OneSignalMessage::create()
            ->subject("Leave approved")
            ->body($this->user->name . " mengajukan $getType->type_name diterima oleh ". $this->approvedBy->name)
            ->setData("type", "Leave")
