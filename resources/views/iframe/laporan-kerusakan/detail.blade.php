...
<form class="kt-form" method="POST" action="{{route('laporan-kerusakan.approve',[$user->id, $data->id])}}">
@csrf
<div class="kt-portlet__body">
    ...
    <div class="form-group">
        ...
        <div class="kt-section__content">{{$data->nama_kapal}}</div>
    </div>
    <div class="form-group">
                    ...
                    @if (count($data->details)>0)
                        @foreach ($data->details as $detail)
                            <tr>
                                <th>{{ $loop->index+1 }}</th>
                                <td>
                                    {{$detail->posisi_di_kapal}}
                                </td>
                                <td>{{$detail->jumlah_kerusakan}}</td>
                                <td>
                                    {{$detail->uraian_kerusakan}}
                                </td>
                                <td>
                                    {{$detail->analisis_kerusakan}}
                                </td>
                                <td>
                                    {{$detail->usaha_penanggulangan}}
                                </td>
                                <td>
                                    {{$detail->hal_yang_perlu_ditindak_lanjuti}}
                                </td>
                            </tr>
                        @endforeach
                    @else
...
<!--end::Modal-->
<form class="kt-form" method="POST" action="{{route('laporan-kerusakan.reject',[$user->id, $data->id])}}">
    @csrf
    <!--begin::Modal reject-->
