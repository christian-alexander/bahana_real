...
@section('body')
<form class="kt-form" id="form-store" method="POST" action="{{route('laporan-penangguhan-pekerjaan.update',[$user_id,$data->id])}}">
    @csrf
    <div class="kt-portlet__body">
        ...
        <div class="form-group">
            ...
            <input type="text" class="form-control" disabled value="{{$data->laporanKerusakan->nomor}}">
        </div>
        <div class="form-group">
            <label for="exampleSelect1">Nama Kapal</label>
            <input type="text" class="form-control" name="no" disabled value="{{$data->laporanKerusakan->nama_kapal}}">
        </div>
        ...
        <div class="form-group">
            ...
            <select class="form-control" id="my-select2" name="bagian_kapal">
                <option value="dek" {{$data->bagian_kapal=='dek'?'selected':''}}>Dek</option>
                <option value="mesin" {{$data->bagian_kapal=='mesin'?'selected':''}}>Mesin</option>
            </select>
        </div>
        <div class="form-group">
            ...
            <select class="form-control my-select2" id="exampleSelect1" name="pelaksana">
                @foreach ($listUser as $key => $item)
                    <option value="{{$key}}" {{$data->pelaksana==$key?'selected':''}}>{{$item}}</option>
                @endforeach
            </select>
        </div>
        <div class="form-group row">
                    ...
                    <tbody>
                            @forelse ($data->details as $item)
                                <tr>
                                    ...
                                    <td>
                                        {{$item->item_pekerjaan}}
                                        <input type='hidden' name="item_pekerjaan[]" value="{{$item->item_pekerjaan}}">
                                    </td>
                                    <td>
                                        {{$item->posisi}}
                                    ...
                                    <td>
                                        {{$item->alasan_penangguhan}}
                                        <input type='hidden' name="alasan_penangguhan[]" value="{{$item->alasan_penangguhan}}">
                                    </td>
                                    <td>
                                        {{$item->target_perbaikan}}
                                        <input type='hidden' name="target_perbaikan[]" value="{{$item->target_perbaikan}}">
                                    </td>
                                    <td>
                                        {{$item->keterangan}}
